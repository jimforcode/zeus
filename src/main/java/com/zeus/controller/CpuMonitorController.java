...
public class CpuMonitorController extends BaseController {
    ...
    public Map<String, Object> getCpuInfo(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (auth == null) {
            auth = cpuMonitorService.getAuth(Constants.USER_NAME, Constants.PASS_WORD);
        ...
        // 3.获取该指定主机的监控项itemId
        String itemId = cpuMonitorService.getItemId(hostId, Constants.CPU_SYSTEM, auth);

        // 4.获取该监控项的监控数据
        JSONArray result = cpuMonitorService.getCpuMonitorInfo(itemId, null, null, auth);
